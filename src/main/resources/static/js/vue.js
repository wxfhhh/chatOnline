...
let app = new Vue({
        ...
        methods:{
                ...
                //解散群聊
                delete_group(){
                        alert(this.my_groups[this.chatting_group_index].owner_id);
                        if(this.my_groups[this.chatting_group_index].owner_id.toString()!==this.user_id.toString()){
                                alert("只有群主才能进行此操作！");
                                return;
                        }
                ...
                search_group(){
                        ...
                        axios.post('/search_group',{
                                ...
                                if(response.data.status===200){
                                        that.group_search_result = response.data.data;
                                }
                        }).catch(function (e) {
                                console.log(e);
                        });
                ...
                change_group_info_panel(){
                  ...
                  axios.post('/get_group_members',{
                          ...
                          if(response.data.status===200){
                                  that.group_members = response.data.data;
                          }
                           ...
                           axios.post('/get_group_messages',{
                                   ...
                                   if(response.data.status===200){
                                           ...
                                           for (let j = 0; j < messages.length; j++) {
                                                   if (that.isImage(messages[j].content)) {
                                                           // 如果是图片类型的消息，生成相应的HTML标记
                                                           messages[j].contentHTML = `<a href="/download?name=${encodeURIComponent(messages[j].content)}" download><img src="/download?name=${encodeURIComponent(messages[j].content)}" alt="image" width="200" height="150"></a>`;
                                                   } else if (that.isOtherFile(messages[j].content)) {
                                                           // 如果是其他文件类型的消息，生成相应的HTML标记
                                                           messages[j].contentHTML = `<a href="/download?name=${encodeURIComponent(messages[j].content)}" download>${decodeURIComponent(messages[j].content)}</a>`;
                                                   } else {
                                                           // 否则为文本类型的消息，直接使用内容
                                                           messages[j].contentHTML = messages[j].content;
                                                   }
                                           }
                                           that.my_groups[i].messages = messages;//
                                           if(messages.length>0)
                                                that.my_groups[i].lastMessage = messages[messages.length-1].member_name+":"+messages[messages.length-1].contentHTML;//显示最新一条消息
                                   }
                           }).catch(function (error) {
                                   console.log(error);
                           })
                ...
                put_group_member_id_in_array(){
                  for(let i=0;i<this.my_groups.length;i++){
                        ...
                        axios.post('/get_members_id',{
                                ...
                                if(response.data.status===200){
                                        that.my_groups[i].ids=response.data.data;
                                }
                        }).catch(function (e) {
                                console.log(e);
                        })
                ...
                send_lobby_message(){
                        if(this.lobby_text_typing=="" && this.typing_file==""){
                                alert('不能发送空消息！');
                                return;
                        }
                        let t;
                        ...
                        if(!this.lobby_or_group){
                                ...
                                let message = {name:this.user_id+"|"+this.my_nickname,url:this.profile_url,time:timeNow,content:t,type:2,}
                                ...
                                if (this.isImage(t)) {
                                        // 如果是图片类型的消息，生成相应的HTML标记
                                        msg.contentHTML = `<a href="/download?name=${encodeURIComponent(t)}" download><img src="/download?name=${encodeURIComponent(t)}" alt="image" width="200" height="150"></a>`;
                                } else if (this.isOtherFile(t)) {
                                        // 如果是其他文件类型的消息，生成相应的HTML标记
                                        msg.contentHTML = `<a href="/download?name=${encodeURIComponent(t)}" download>${decodeURIComponent(t)}</a>`;
                                } else
                                    // 否则为文本类型的消息，直接使用内容
                                        msg.contentHTML =t;
                        ...
                        else
                        {
                                ...
                                axios.post('/send_group_message',{
                                        ...
                                        if(response.data.status===200)
                                        {
                                                that.my_groups[that.chatting_group_index].lastMessage=that.my_nickname+":"+t;
                                                that.lobby_text_typing = "";
                                                that.typing_file="";
                                                that.scroll();
                                        }else
                        ...
                        axios.post('/get_lobby_messages',{}).then(function (response) {
                                if(response.data.status===200){
                                        ...
                                        for(let i=0;i<messages.length;i++){
                                                ...
                                                if (that.isImage(messages[i].content)) {
                                                        // 如果是图片类型的消息，生成相应的HTML标记
                                                        message.contentHTML = `<a href="/download?name=${encodeURIComponent(messages[i].content)}" download><img src="/download?name=${encodeURIComponent(messages[i].content)}" alt="image" width="200" height="150"></a>`;
                                                } else if (that.isOtherFile(messages[i].content)) {
                                                        // 如果是其他文件类型的消息，生成相应的HTML标记
                                                        message.contentHTML = `<a href="/download?name=${encodeURIComponent(messages[i].content)}" download>${decodeURIComponent(messages[i].content)}</a>`;
                                                } else {
                                                        // 否则为文本类型的消息，直接使用内容
                                                        message.contentHTML = messages[i].content;
                                                }
                ...
                //发送消息
                send_message(){
                        if(this.typing_text==="" && this.typing_file==""){
                                alert('不能发送空消息！');
                                return;
                        }
                        if(this.chatting_with===-1){
                        ...
                        if (this.typing_text===""){
                                ...
                                t=this.typing_text
                        //通过web socket对方发送时时消息
                        let message = {from:this.user_id,to:this.friends[this.chatting_with].id.toString(),text:t,type:1}
                        socket.send(JSON.stringify(message));
                        //显示到本地
                        let msg = [];
                        msg.type = 'out';
                        ...
                        this.friends[this.chatting_with].lastChat = t;
                ...
                handleAvatarSuccess(event) {
                        ...
                        const maxSizeInBytes = 10 * 1024 * 1024; // 10MB (adjust the value as needed)

                        if (file.size > maxSizeInBytes) {
                                console.log('文件不能超过10MB');
                                return;
                        }
                        this.org_typing_file=file;
                        ...
                        axios.post('/upload', formData)
                            .then(response => {
                                    if (response.data.status ===200) {
                                            this.typing_file = response.data.data;
                                            console.log(this.typing_file);
                                    } else {
                                ...
                                axios.post('/get_message',{
                                        ...
                                        if(response.data.status===200){
                                                ...
                                                for(let j=0;j<messages.length;j++){
                                                        ...
                                                        if (that.isImage(messages[j].content)) {
                                                                // 如果是图片类型的消息，生成相应的HTML标记
                                                                message.contentHTML = `<a href="/download?name=${encodeURIComponent(messages[j].content)}" download><img src="/download?name=${encodeURIComponent(messages[j].content)}" alt="image" width="200" height="150"></a>`;
                                                        } else if (that.isOtherFile(messages[j].content)) {
                                                                // 如果是其他文件类型的消息，生成相应的HTML标记
                                                                message.contentHTML = `<a href="/download?name=${encodeURIComponent(messages[j].content)}" download>${decodeURIComponent(messages[j].content)}</a>`;
                                                        } else {
                                                                // 否则为文本类型的消息，直接使用内容
                                                                message.contentHTML = messages[j].content;
                                                        }
                ...
                renderMessageContent(content) {
                        if (this.isImage(content)) {
                                return `<a href="/download?name=${encodeURIComponent(content)}" download><img src="/download?name=${encodeURIComponent(content)}" alt="image"></a>`;
                        } else if (this.isOtherFile(content)) {
                                return `<a href="/download?name=${encodeURIComponent(content)}" download>${decodeURIComponent(content)}</a>`;
                        } else {
                                return content;
                        }
                },
                isImage(content) {
                        const imageExtensions = ['.jpg', '.jpeg', '.png', '.gif']; // 可根据实际情况添加更多的图片扩展名
                        const fileExtension = content.substring(content.lastIndexOf('.')).toLowerCase();
                        return imageExtensions.includes(fileExtension);
                },
                isOtherFile(content) {
                        const otherFileExtensions = ['.doc', '.docx',]; // 可根据实际情况添加更多的图片扩展名
                        const fileExtension = content.substring(content.lastIndexOf('.')).toLowerCase();
                        return otherFileExtensions.includes(fileExtension);
                },
                // downloadFile(fileName, friendIndex, messageType) {
                //         axios.get('/download', {
                //                 params: {
                //                         name: fileName
                //                 },
                ...
                //                 };
                //                 this.friends[friendIndex].messages.push(message);
                ...
                get_my_info(){
                        ...
                        axios.post('/get_my_info',{
                                ...
                                if(response.data.status===200){
                                        that.nickname_edit = that.my_nickname = response.data.data.nickname;
                                        that.my_email = response.data.data.email;
                                        that.sex_edit = that.my_sex=response.data.data.sex;
                                        that.phone_edit = that.my_phone =  response.data.data.phone;
                                        that.birthday_edit = that.my_birthday = response.data.data.birthday;
                                        that.signature_edit = that.my_signature = response.data.data.signature;
                                        that.profile_url = response.data.data.url;
                                        that.register_time = response.data.data.register_time;
                                }
                                else{
                                        alert("用户信息已失效，请重新登录！");
                                        window.location.href="/logout";
